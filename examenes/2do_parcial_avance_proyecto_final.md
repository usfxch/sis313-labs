...
## 📝 Notas para el Docente — Guía de Evaluación en 10 Minutos

### Logística sugerida

1. **Entrega en eCampus:** Verificar que el grupo haya subido los 3 documentos antes de las 09:00. Quienes no cumplan este requisito no defienden y obtienen nota 0 en el parcial.
2. **Sorteo de horarios:** A las 10:00 se sortea el orden de presentación. Los grupos deben estar presentes y atentos. Los que no estén listos en su horario asignado pierden 5 puntos por cada 5 minutos de retraso.
3. **Llegada lista:** Exigir que las VMs estén encendidas antes de entrar. Si un grupo pierde tiempo encendiendo o iniciando sesión, corre por su cuenta.
4. **Control de tiempo:** Usar un cronómetro visible. Avisar cuando falten 2 minutos. A los 10 minutos, detener la presentación.

### Distribución sugerida del tiempo del docente

| Minuto | Acción del docente |
|--------|--------------------|
| 0-0:30 | Verifica identidad del grupo, proyecto y confirma que subió documentos a eCampus. |
| 0:30-1:30 | Hito 1: pide `ip addr` y un `ping` rápido. |
| 1:30-5:30 | Hito 2: el grupo demuestra sus 2 servicios. El docente pide una prueba específica (ej. "hazme un dig", "muéstrame el failover"). |
| 5:30-7:00 | Hito 3: verificación rápida de SSH, firewall y permisos. |
| 7:00-9:00 | Hito 4: revisa el diagrama, escucha la defensa de cada uno (15 seg por persona). |
| 9:00-10:00 | Pregunta puntual (máximo 1) y calificación. |

### Preguntas de defensa rápidas (elegir 1 o 2)

Para validar que todos participaron, sin extender el tiempo:

1. *"Integrante A: ¿qué IP tiene tu VM de base de datos y en qué archivo la configuraste?"*
2. *"Integrante B: muéstrame el comando que usaste para verificar que el firewall está activo."*
3. *"Integrante C: si apago esta VM, ¿qué parte del proyecto deja de funcionar?"*
4. *"Integrante D: ¿qué letra de la bitácora corresponde a tu tarea y qué problema tuviste?"*

### Criterio de aprobación mínima

Un grupo que no tenga funcionando **ningún servicio del Hito 2** debe calificarse con nota menor a 51/100, independientemente de los demás hitos. La infraestructura sola no es suficiente para aprobar.

### Flexibilidad por proyecto

Si un grupo eligió un proyecto atípico (ej. SOC, Disaster Recovery, etc.), adapte las "Opciones" del Hito 2 manteniendo la **complejidad equivalente**. La clave es que demuestren 2 temas avanzados funcionando, no que cumplan textualmente la tabla.
